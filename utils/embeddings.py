...
class PolicyEmbeddings:
    ...
    def load_documents(self, docs_path: str) -> List[Dict[str, Any]]:
        ...
        if has_changed:
            self.logger.info("Policy changes detected or no existing data, processing documents")
    ...
    def add_document(self, content: str, source: str, save_immediately: bool = True) -> int:
    ...
    def remove_documents_by_source(self, source: str, save_immediately: bool = True) -> int:
    ...
    def get_storage_stats(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(self.metadata_path):
                stats['metadata_size_mb'] = round(os.path.getsize(self.metadata_path) / (1024 * 1024), 2)
            else:
    ...
    def optimize_index(self):
        ...
        try:
            ...
            if current_size > 1000:
                self.logger.info(f"Collection has {current_size} points, optimization may improve performance")
                ...
                self.logger.info("Consider configuring HNSW parameters for large collections")
                self.logger.info("Qdrant automatically handles index optimization in the background")
            else:
